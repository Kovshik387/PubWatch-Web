...
export default function ProfilePage() {
    ...
    const addVoluteData = async () => {
        if (newCurrency) {
            await AccountService.addVolute(id!, newCurrency.name);
        }
    }

    const removeVoluteData = async (currency: Favorite) => {
        await AccountService.deleteVolute(id!, currency.name);
    }
    ...
    const fetchAccount = async () => {
        ...
        try {
            ...
            setSelectedOption(response.data.accept);
        } catch (error) {
    ...
    const handleUploadPhoto = async () => {
        if (selectedFile) {
            ...
            setAccount({ ...account!, image: response.data.data });
            handleCloseModal();
        }
    };
    ...
    const handleDeletePhoto = async () => {
        if (account?.image != ""){
            const id = searchParams["id"];
            await AccountService.deleteImage(id!);    
        }
        handleCloseModal();
    }
                            ...
                            <Col>
                                ...
                                <Form hidden={!edit}>
                                    <Form.Check
                                        ...
                                        readOnly={!edit}
                                        className="text-secondary custom-switch"
                                    />
                                </Form>
                            </Col>
                        </Row>
                                ...
                                <InputGroup hidden={!edit}>
                                    <Typeahead
                                        ...
                                        onChange={(selected) => {
                                            ...
                                            if (selectedVolute) {
                                                setNewCurrency({ name: selectedVolute.name});
                                            } else {
                                                setNewCurrency(null);
                                            }
                                        }}
                        ...
                        <Row className="mb-4">
                            <Col xs={12}>
                                {account?.favorites.map((currency, index) => (
                                    <Badge key={index} style={{background: "#9b9bbd"}} bg="#9b9bbd" className="mb-2 me-2" >
