...
export default class AuthService {
    static async signIn(email: string,password: string, device: string = navigator.userAgent): Promise<AxiosResponse<AuthResponse>> {
        return $api.post<AuthResponse>("auth/signIn",{email,password, device})
    }
